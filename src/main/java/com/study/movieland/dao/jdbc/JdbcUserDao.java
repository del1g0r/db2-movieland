...
public class JdbcUserDao implements UserDao {
    ...
    private static final String GET_ALL_SQL = "SELECT id, name, role_name FROM \"user\"";
    private static final String CHECK_PWD_SQL = "SELECT pswhash = crypt(?, salt) is_checked, id, name, role_name FROM \"user\" WHERE name = ?";
    private static final String CREATE_SQL = "INSERT INTO \"user\" (name, salt, pswhash, role_name) SELECT ?, t.md5, crypt(?, t.md5), 'GUEST' FROM (SELECT gen_salt('md5') md5) t";
    private static final String GET_SQL = "SELECT id, name, role_name FROM \"user\" WHERE id = ?";
    private static final String GET_BY_NAME_SQL = "SELECT id, name, role_name FROM \"user\" WHERE name = ?";
